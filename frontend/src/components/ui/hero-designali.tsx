...
function onMousemove(e) {
    ...
    // Autonomous animation logic
    function animate() {
        // @ts-ignore
        if (ctx.running) {
            // Update position autonomously based on time
            const time = Date.now() * 0.001;
            // @ts-ignore
            pos.x = Math.sin(time) * (window.innerWidth * 0.3) + window.innerWidth / 2;
            // @ts-ignore
            pos.y = Math.cos(time * 0.5) * (window.innerHeight * 0.2) + window.innerHeight / 2;
        }
